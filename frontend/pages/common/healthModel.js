export default {
  ...
  updateHealthMetrics({ weight, height }) {
    if (weight) this.data.weight = weight;
    if (height) this.data.height = height;
  ...
  calculateBMI() {
    ...
    // 更新 weightStatus
    if (this.data.bmi < 18.5) this.data.weightStatus = "underweight";
    else if (this.data.bmi < 24) this.data.weightStatus = "normal";
    else if (this.data.bmi < 28) this.data.weightStatus = "overweight";
    else this.data.weightStatus = "obese";
  },

  // 更新步数数据
  updateSteps(newSteps) {
    this.data.steps = newSteps;
    this.data.lastUpdate = new Date().toLocaleDateString();
  },
